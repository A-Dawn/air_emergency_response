...
class StringEncoded(TypeDecorator):
    impl = String(255)  # Specify a length for the String type
    cache_ok = True
...
class SM4EncryptedType(TypeDecorator):
    ...
    def __init__(self, key, *args, **kwargs):
        self.key = key
        # 使用密钥的MD5值的前16个字符作为IV
        self.iv = hashlib.md5(key.encode('utf-8')).hexdigest()[:16]
        super().__init__(*args, **kwargs)
    def process_bind_param(self, value, dialect):
        if value is not None:
            from utils.sm_utils import encrypt_sm4
            value = encrypt_sm4(self.key, value,self.iv)
        return value
    def process_result_value(self, value, dialect):
        if value is not None:
            from utils.sm_utils import decrypt_sm4
            value = decrypt_sm4(self.key, value,self.iv)
        return value
class Summary(db.Model):
    ...
    content = db.Column(SM4EncryptedType(key=hashlib.md5(b'content').hexdigest()), nullable=False)
